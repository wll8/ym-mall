    ...
    >
      <view class="goods-content">
        <view class="goods-list">
          <view class="goods-item" v-for="item in data?.orderGrouponGoods" :key="item.id">
            <view class="goods-item-top flex">
              <view class="goods-cover">
                <image mode="widthFix" class="cover-img" :src="item?.goodsImage" />
              </view>
              <view class="goods-info">
                ...
                <view class="info-tips">
                  <view class="info-tip">
                    ...
                    <view class="info-tip-right">{{ item?.goodsSpec }}</view>
                  </view>
                  <view class="info-tip">
                    <view class="info-tip-left">数量：</view>
                    <view class="info-tip-right">{{ item?.goodsNum }}</view>
                  </view>
                </view>
              </view>
            </view>
            <address-card-v2
              ...
              mt2
              :name="item?.consignee"
              :phone="item?.consigneePhoneNumber"
              :detailAddress="getAddress(item)"
            />
          </view>
        </view>
        <view class="goods-count" v-if="!isHidePrice">
          <view class="goods-tips">共 {{ goodsNum }} 件商品，合计：</view>
...
const goodsNum = computed(() => {
  if (props.data && props.data.id) {
    return props.data?.orderGrouponGoods.reduce((pre: number, cur: any) => {
