...
<template>
  <view class="goods-edit">
    ...
    <FooterButtonCard>
      <CommonButton width="100%" @click="editGoods">确定</CommonButton>
    </FooterButtonCard>
  </view>
</template>
...
function getPromotionRate() {
  getSupplerInfoById(goodsForm.supplierId).then((res) => {
    ...
    if (isCooperation.value === '1') {
      ...
      // 计算销售价
      if (goodsForm.promotionRate && goodsForm.costPrice) {
        calcSalePriceByCostPriceAndPromotionRate()
        // 计算毛利率
      } else if (goodsForm.salePrice && goodsForm.costPrice) {
  ...
  getGoodsDetail(id.value).then((res) => {
    if (String(res.code) === '200') {
      ...
      for (const key of keys) {
        ...
        if (key === 'promotionRate' && res.data[key] == null) {
          continue
        }
        if (key === 'isReturn' && res.data[key] == null) {
          goodsForm[key] = '1'
          continue
        }
        if (key === 'alertStock') {
          goodsForm[key] = String(res.data[key] || '1')
          continue
        }
      ...
      // 商品库存
      goodsForm.totalStock = String(goodsForm.totalStock)
      // 处理商品分类
...
function editGoods() {
  formRef.value
    .validate()
    .then(() => {
      const info = { ...detailData.value, ...goodsForm }
      // 处理商品分类
      ...
      updateGoodsForSupplier(info).then((res) => {
