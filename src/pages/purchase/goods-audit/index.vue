...
<template>
  <view class="goods-audit">
    <view class="main">
      ...
      >
        ...
        <uv-form-item label="无理由退货" prop="isReturn" borderBottom>
          <uv-radio-group
            disabled
            :modelValue="goodsForm.isReturn"
            @update:modelValue="updateIsReturn"
          >
        ...
        <uv-form-item label="商品参数" prop="goodsParamsList" label-position="top" borderBottom>
          <AddGoodsParams v-model="goodsForm.goodsParamsList" disabled />
        </uv-form-item>
        <uv-form-item label="商品封面图" label-position="top" prop="goodsCover">
          <view class="wrap-upload">
            <UploadImages
              ...
              :show-file-list="photoBackShow.goodsCoverFileList"
              disabled
            />
          </view>
        </uv-form-item>
        <uv-form-item label="商品主图" label-position="top" prop="slideshowList">
          <view class="wrap-upload">
            <UploadImages
              v-model:imgs-list="goodsForm.slideshowList"
              :show-file-list="photoBackShow.slideshowList"
              disabled
            />
          </view>
        </uv-form-item>
        <uv-form-item label="图文详情" label-position="top" prop="detailImgUrlList">
          <view class="wrap-upload">
            <UploadImages
              v-model:imgs-list="goodsForm.detailImgUrlList"
              :show-file-list="photoBackShow.detailImgUrlList"
              disabled
            />
          </view>
        </uv-form-item>
    ...
    <FooterButtonCard>
      <view class="buttons justify-between" w-full flex>
        <uv-button type="success" @click="agree" text="通过" style="width: 48%"></uv-button>
        <uv-button type="error" @click="rejected" text="拒绝" style="width: 48%"></uv-button>
      </view>
    </FooterButtonCard>
    <modal-enter
      title="审核理由"
...
import { extractImageUrlsFromHtml } from '@/utils/ritchtext-to-imgs'
...
// 图片回显信息
const photoBackShow = reactive<any>({
  goodsCoverFileList: [],
  slideshowList: [],
  detailImgUrlList: [],
})
...
function getGoodsInfo() {
  getGoodsDetail(id.value).then((res) => {
    if (String(res.code) === '200') {
      ...
      // 赋值给表单
      goodsForm.detailContent = detailData.value.detailContent // 富文本内容

      const keys = Object.keys(goodsForm)
      for (const key of keys) {
        if (key === 'goodsParamsList' || key === 'goodsCategoryId') {
          continue
        }
        goodsForm[key] = res.data[key]
      }
      const goodsCategoryIds = res.data.goodsCategoryId.split('-')
      // 获取goodsCategoryId
      goodsForm.goodsCategoryId = goodsCategoryIds[goodsCategoryIds.length - 1]
      // 处理商品参数
      if (res.data.goodsParamsList && res.data.goodsParamsList.length) {
        goodsForm.goodsParamsList = res.data.goodsParamsList
      } else {
        goodsForm.goodsParamsList = [
          {
            paramName: '',
            paramValue: '',
          },
      ...
      goodsForm.id = id.value

      // 封面图片
      goodsForm.goodsCover = [goodsForm.goodsCover]
      photoBackShow.goodsCoverFileList = handlePhotoBack(goodsForm.goodsCover)
      // 商品主图
      photoBackShow.slideshowList = handlePhotoBack(goodsForm.slideshowList)
      const images = extractImageUrlsFromHtml(goodsForm.detailContent)
      // 图文详情处理
      goodsForm.detailImgUrlList = images
      // 图文详情图片列表
      photoBackShow.detailImgUrlList = handlePhotoBack(images)
      goodsForm.activity = String(goodsForm.activity)
      // 处理商品分类
      const goodsCIds = goodsForm.goodsCategoryId.split('-')
      goodsForm.goodsCategoryId = goodsCIds[goodsCIds.length - 1]
    }
  })
}
// 回显图片文件列表处理
function handlePhotoBack(list: string[]) {
  const newList = []
...
function calcSalePriceByCostPriceAndPromotionRate() {
  goodsForm.salePrice = (+goodsForm.costPrice * (1 + +goodsForm.promotionRate / 100)).toFixed(2)
}
// 通过成本价和和销售价计算毛利率
function calcPromotionRateByCostPriceAndSalePrice() {
  goodsForm.promotionRate = ((+goodsForm.salePrice / +goodsForm.costPrice - 1) * 100).toFixed(0)
}
// 更新成本价
...
function returnGoodsPriceByReturnGoodsRateAndSalePrice() {
  goodsForm.returnGoodsPrice = ((+goodsForm.returnGoodsRate / 100) * +goodsForm.salePrice).toFixed(
    2,
  )
}
// 通过退货价和销售价计算退货比率
function returnGoodsRateByReturnGoodsPriceAndSalePrice() {
  goodsForm.returnGoodsRate = ((+goodsForm.returnGoodsPrice / +goodsForm.salePrice) * 100).toFixed()
}
// 更新销售价
...
function agree() {
  // 审核提示
  if (goodsForm.salePrice == null) {
    uni.showModal({
      title: '提示',
      content: `销售价未填写，请编辑商品后填写后，审核`,
      showCancel: false,
    })
    return false
  }
