    ...
    <view class="header">
      <view class="fix-top" :style="{ top: headTop }">
        ...
        <view class="tabs-area">
          <TabsV2 :tabs="tabs" :index="tableCurrentIndex" @tab-change="changeSearchType" />
        </view>
      </view>
    </view>
      ...
      <view class="order-item" v-for="item in showList" :key="item.id">
        <template v-if="item.orderState === 1">
          <CommodityCardV4
            ...
            :data="item"
            :header-option="logisticsProgressMapShowInfos[item.logisticsProgress]"
...
const tableCurrentIndex = ref(0)

const logisticsProgressMapShowInfos = reactive({
  1: {
    rightColor: '#FEA800',
    rightText: '供应商待发货',
  },
...
onLoad((options) => {
  ...
  queryInfo.buyerId = userStore.userInfo.user_id
  if (options.type) {
    queryInfo.orderState = Number(options.type)
    tableCurrentIndex.value = tabs.findIndex((item) => item.value === queryInfo.orderState)
  }
  initCurrentData()
})
onShow(() => {
...
function toOrderDetail(item) {
  uni.navigateTo({
    url: `/pages/purchase/order/order-detail/index?id=${item.id}`,
  })
}
