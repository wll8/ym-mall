...
<template>
  <view class="supplier-goods page-background">
    <view class="header">
    ...
    <view class="main">
      <template v-if="showList.length">
        <GoodsList :data="showList" @cardClick="toEditGoods">
          <template #default="scope">
            <view class="item-content" v-if="currentTabValue !== -1" w-full>
              <view class="buttons" flex justify-end>
                <view
                  class="btn-item"
                  v-if="String(scope.row.auditStatus) === '1' && String(scope.row.isSale) === '0'"
                >
                  <uv-button
                    type="primary"
                    :plain="true"
                ...
                <view
                  class="btn-item"
                  v-if="String(scope.row.auditStatus) === '1' && String(scope.row.isSale) === '1'"
                >
                  <uv-button
                    type="primary"
                    :plain="true"
                ...
                <view class="btn-item" v-if="String(scope.row.auditStatus) === '0'">
                  <uv-button
                    type="success"
                    :plain="true"
                    @click="toGoodsAudit(scope.row)"
                    text="审核"
                  ></uv-button>
                </view>
                <view
                  class="btn-item"
                  v-if="String(scope.row.auditStatus) === '0' || String(scope.row.isSale) === '0'"
                >
                  <uv-button
                    type="error"
                    :plain="true"
              ...
              <view
                ...
                w-full
                v-if="scope.row.auditStatus == 2 && scope.row.auditMessage"
              >
...
import {
  deleteGoodsForSupplier,
  getGoodsListBySupplier,
  whetherPublish,
  changeInventory,
} from '@/service/goods'
...
function changeSearchType(info: any) {
  currentTabValue.value = info.value
  switch (info.value) {
    case -1: // 全部
      queryInfo.auditStatus = undefined
      queryInfo.isSale = undefined
      queryInfo.isNotEnough = undefined
      break
    case 0: // 审核中
      queryInfo.auditStatus = 0
      queryInfo.isSale = undefined
      queryInfo.isNotEnough = undefined
      break
    case 1: // 已通过
      queryInfo.auditStatus = 1
      queryInfo.isSale = undefined
      queryInfo.isNotEnough = undefined
      break
    case 2: // 未通过
      queryInfo.auditStatus = 2
      queryInfo.isSale = undefined
      queryInfo.isNotEnough = undefined
      break
    case 3: // 上架
...
onLoad((options) => {
  if (options.userId) {
    queryInfo.supplierId = options.userId
  }
})
...
function getShowList() {
  queryInfo.current += 1
  getGoodsListBySupplier(queryInfo).then((res: any) => {
...
function toAddGoods() {
  uni.navigateTo({
    url: `/pages/purchase/goods-add/index?supplierId=${queryInfo.supplierId}`,
  })
}
/**
 * 编辑商品
...
function toEditGoods(item: any) {
  uni.navigateTo({
    url: `/pages/purchase/goods-edit/index?supplierId=${queryInfo.supplierId}&id=${item.id}`,
  })
}
// 去审核商品
function toGoodsAudit(item: any) {
  uni.navigateTo({
    url: `/pages/purchase/goods-audit/index?supplierId=${queryInfo.supplierId}&id=${item.id}`,
  })
}
// 删除商品
function onDeleteGoods(item: any) {
  uni.showModal({
    ...
    success: (res) => {
      if (res.confirm) {
        deleteGoodsForSupplier(item.id).then((res) => {
          if (res.code === 200) {
            uni
              .showToast({
                title: res.msg,
...
function onShelf(item: any, isOnSelf = true) {
  uni.showModal({
    ...
    success: (res) => {
      if (res.confirm) {
        whetherPublish({ ids: item.id, state: isOnSelf ? 1 : 0 }).then((res) => {
          showToast(res.msg)
          initCurrentData()
        })
      }
