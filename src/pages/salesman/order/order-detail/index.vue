  ...
  <view class="orderdetail page-background">
    ...
    <view class="order-item" v-if="orderInfo.orderState == 6 || orderInfo.orderState == 7">
      <CellsCard title="退货信息" :data="returnInfo" :columns="returnInfoColumns">
        ...
        <template #returnImg>
          <ShowImgs :data="mapImgs(returnInfo?.returnImg.split(','))" />
        </template>
        <template #busiImg>
          <ShowImgs :data="mapImgs(returnInfo?.busiImg.split(','))" />
        </template>
      </CellsCard>
    </view>
    <view class="order-item">
    ...
    <view class="order-item" v-if="orderInfo.orderState === 4">
    ...
    >
      <CommonButton width="100%" @click="openRefoundRequest">申请处理</CommonButton>
    </FooterButtonCard>
...
function openRefoundRequest() {
  refundApplicationRef.value.open(orderInfo.value, returnInfo.value)
}
// 退货申请成功
